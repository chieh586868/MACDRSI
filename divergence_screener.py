...
# ── 排除冷門產業（沿用 macd_app 的關鍵字比對 + TWSE/TPEx 產業 API）──
...
def fetch_industry_map():
    """建 dict[sid -> 產業名稱]，存 industry_map.json，7 天 cache。抓不到不影響掃描。"""
    global industry_map
    if os.path.exists(INDUSTRY_FILE):
        try:
            if (time.time() - os.path.getmtime(INDUSTRY_FILE)) / 86400 < 7:
                with open(INDUSTRY_FILE, encoding="utf-8") as f:
                    data = json.load(f)
                if len(data) > 100:
                    with industry_lock:
                        industry_map = data
                    print(f"  ✅ 產業 map 從 cache 載入：{len(data)} 檔")
                    return
        except Exception:
            pass
    out = {}
    ...
    if len(out) > 100:
        ...
        with industry_lock:
            industry_map = out
        excl = sum(1 for v in out.values() if is_excluded_industry(v))
        print(f"  ✅ 產業 map：{len(out)} 檔（排除產業命中 {excl} 檔）")
    else:
        print("  ⚠ 產業 map 抓取過少，略過（不影響掃描，僅『排除冷門產業』暫時失效）")


def _filter_stocks(stocks, exclude_traditional):
    """exclude_traditional=True 時濾掉冷門產業股；產業 map 未載入則原樣回傳。"""
    if not exclude_traditional:
        return stocks
    with industry_lock:
        m = industry_map
    if not m:
        return stocks
    return [s for s in stocks if not is_excluded_industry(m.get(s.get("id", ""), ""))]
...
# ── 全市場掃描 ────────────────────────────────────────────
def run_scan(mode="daily", min_score=2, div_type="all", exclude_traditional=False):
    global scan_progress
    stocks = _filter_stocks(list(watchlist), exclude_traditional)
    total  = len(stocks)
...
def run_dw_scan(mode="triple", min_weekly=2, exclude_traditional=False):
    global scan_progress
    stocks = _filter_stocks(list(watchlist), exclude_traditional); total = len(stocks)
...
def run_cond_scan(cond, exclude_traditional=False):
...
<div class="scan-bar">
  ...
  <label style="display:flex;align-items:center;gap:6px;font-size:13px;color:#9bb3cc;cursor:pointer;user-select:none">
    <input type="checkbox" id="chk-exclude" checked style="width:15px;height:15px;cursor:pointer">
    排除冷門產業
    <span style="font-size:11px;color:#4a6080">(水泥/塑膠/紡織/營建/航運/造紙/觀光/橡膠)</span>
  </label>
...
function excludeParam() {
  const el = document.getElementById('chk-exclude');
...
function startScan(mode) {
  ...
  fetch(`/api/scan?mode=${mode}&type=${type}&min_score=${score}&exclude_traditional=${excludeParam()}`)
    .then(r => r.json())
    .then(data => {
      clearInterval(progTimer);
...
function startDWScan(mode) {
  ...
  fetch(`/api/scan/dw?mode=${mode}&exclude_traditional=${excludeParam()}`)
    .then(r => r.json())
    .then(data => {
      clearInterval(progTimer);
      document.getElementById('prog').style.width = '100%';
...
function startCondScan(cond) {
  ...
  fetch(`/api/cond/${cond}/scan?exclude_traditional=${excludeParam()}`)
    .then(r => r.json())
    .then(data => {
      clearInterval(progTimer);
      document.getElementById('prog').style.width = '100%';
